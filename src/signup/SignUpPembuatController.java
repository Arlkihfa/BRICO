...
public class SignUpPembuatController implements Initializable {
    @FXML
    ...
    void handleButtonRegisterUser(ActionEvent event) {
        if (!usernamePendaftar.getText().equals("") && !passPendaftar.getText().equals("")) {
            ArrayList<DataPembuat> listUser = new ArrayList<>();
            ...
            listUser.add(new DataPembuat(username, pass, nama, tetl, jenisKelamin));
            String xml = xstream.toXML(listUser);
            try {
                dataUser = new FileOutputStream("DataPembuat.xml");
