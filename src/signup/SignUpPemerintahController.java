...
public class SignUpPemerintahController implements Initializable {
    @FXML
    ...
    void handleButtonRegisterUser(ActionEvent event) {
        if (!usernamePendaftar.getText().equals("") && !passPendaftar.getText().equals("")) {
            ArrayList<DataPemerintah> listUser = new ArrayList<>();
            ...
            listUser.add(new DataPemerintah(username, pass, nama, tetl, jenisKelamin));
            String xml = xstream.toXML(listUser);
            try {
                dataUser = new FileOutputStream("DataPemerintah.xml");
